        ...
        for phase in ['train', 'valid']:
            ...
            for batch in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    # (B, T, D)
                    outputs_c1, hiddens = model_c1(inputs[:, c_list[0], :, :])
                    outputs_c2, hiddens = model_c2(inputs[:, c_list[1], :, :])
                    outputs_c3, hiddens = model_c3(inputs[:, c_list[2], :, :])
...
def predict(cfg, model_path_c1, model_path_c2, model_path_c3, loader, device, save_path,c_list):
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(False):
            ...
            outputs_c2, hiddens = model_c2(inputs[:, c_list[1], :, :])
            outputs_c3, hiddens = model_c3(inputs[:, c_list[2], :, :])
            # outputs_c4, hiddens = model_c4(inputs[:, c_list[3], :, :])
...
def experiment(cfg, fold, use_pretrained, predict_on_private,c_list):
    print(fold)
    set_seed(cfg.seed)
    ...
    if use_pretrained:
        ...
        best_metric = train(
            cfg, datasets, dataloaders, device, model_path_c1, model_path_c2, model_path_c3, c_list
        )

    # make predictions
    test_predictions = {}
    predict(
        cfg, model_path_c1, model_path_c2, model_path_c3, dataloaders['train'], device,
        f'./predictions/{cfg.init_time}/{cfg.task}_train_{"_".join([str(i) for i in fold["train"]])}_r21d_rgb.csv',c_list)
    predict(
        cfg, model_path_c1, model_path_c2, model_path_c3, dataloaders['valid'], device,
        f'./predictions/{cfg.init_time}/{cfg.task}_valid_{"_".join([str(i) for i in fold["valid"]])}_r21d_rgb.csv',c_list)
    test_predictions['public_test'] = predict(
        cfg, model_path_c1, model_path_c2, model_path_c3, dataloaders['public_test'], device,
        f'./predictions/{cfg.init_time}/{cfg.task}_public_test_trained_on_{"_".join([str(i) for i in fold["train"]])}_r21d_rgb.csv',c_list)

    if predict_on_private:
        test_predictions['private_test'] = predict(
            cfg, model_path_c1, model_path_c2, model_path_c3, dataloaders['private_test'], device,
            f'./predictions/{cfg.init_time}/{cfg.task}_private_test_trained_on_{"_".join([str(i) for i in fold["train"]])}_r21d_rgb.csv',c_list)

    return best_metric, test_predictions


def run_kfold(cfg,c_list, use_pretrained=False, predict_on_private=None):
    ...
    best_fold_metrics_and_test_predictions = [experiment(cfg, fold, use_pretrained, predict_on_private,c_list) for fold in folds]
...
if __name__ == "__main__":
    ...
    run_kfold(cfg,[0,1,2] ,use_pretrained, get_cmd_args().predict_on_private)
