...
def train(
        cfg, datasets, dataloaders, device, save_model_path_c1, save_model_path_c2, c_list
    ):
    model_c1 = RNN(
...
def predict(cfg, model_path_c1, model_path_c2, loader, device, save_path,c_list):
...
def experiment(cfg, fold, use_pretrained, predict_on_private,c_list):
    ...
    if use_pretrained:
        ...
        best_metric = train(
            cfg, datasets, dataloaders, device, model_path_c1, model_path_c2, c_list
        )

    # make predictions
    test_predictions = {}
    predict(
        cfg, model_path_c1, model_path_c2, dataloaders['train'], device,
        f'./predictions/{cfg.init_time}/{cfg.task}_train_{"_".join([str(i) for i in fold["train"]])}_r21d_rgb.csv',c_list)
    predict(
        cfg, model_path_c1, model_path_c2, dataloaders['valid'], device,
    ...
    test_predictions['public_test'] = predict(
        cfg, model_path_c1, model_path_c2, dataloaders['public_test'], device,
    ...
    if predict_on_private:
        test_predictions['private_test'] = predict(
            cfg, model_path_c1, model_path_c2, dataloaders['private_test'], device,
