...
def train(
        cfg, datasets, dataloaders, device, save_model_path_c1
    ):
    model_c1 = RNN(
    ...
    for epoch in range(cfg.num_epochs):
        for phase in ['train', 'valid']:
            ...
            for batch in dataloaders[phase]:
                ...
                # print("shape of inputs for training", inputs.shape)
                # print("shape of targets",targets.shape)
                
                input_combined = torch.dstack((inputs[:, 0, :, :],inputs[:, 1, :, :],inputs[:, 2, :, :],inputs[:, 3, :, :]))
                # zero the parameter gradients
                optimizer.zero_grad()
                
                # print("shape of combined inputs for training", input_combined.shape)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    outputs_c1, hiddens = model_c1(input_combined)
...
def predict(cfg, model_path_c1, loader, device, save_path):
...
def experiment(cfg, fold, use_pretrained, predict_on_private):
    ...
    if use_pretrained:
        ...
        best_metric = train(
            cfg, datasets, dataloaders, device, model_path_c1
        )
